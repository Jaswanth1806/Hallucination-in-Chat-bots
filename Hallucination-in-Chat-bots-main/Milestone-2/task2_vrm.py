...
def load_data(task):
    ...
    for i in range(len(data)):
        for j in range(len(data[i]["utterances"])):
            ...
            for k in range(len(data[i]["utterances"][j]["VRM"])):
                if data[i]["utterances"][j]["VRM"][k] == 'Edification' or data[i]["utterances"][j]["VRM"][k] == ' edification' or data[i]["utterances"][j]["VRM"][k] == ' Edification':
                    label[0] = 1
                elif data[i]["utterances"][j]["VRM"][k] == 'Disclosure':
                    label[1] = 1
                elif data[i]["utterances"][j]["VRM"][k] == 'Advisement' or data[i]["utterances"][j]["VRM"][k] == ' Advisement':
                    label[2] = 1
                elif data[i]["utterances"][j]["VRM"][k] == 'Question':
                    label[3] = 1
                elif data[i]["utterances"][j]["VRM"][k] == 'Ack.':
                    label[4] = 1
...
if __name__ == "__main__":
    ...
    # Defining the Model
    model = TFRobertaForSequenceClassification.from_pretrained('roberta-base', num_labels=5, output_attentions=False, output_hidden_states=False)
   ...
   # Creating test dataset
    ...
    for i, label in enumerate(['Edification', 'Disclosure', 'Advisement', 'Question', 'Ack.']):
        print("Label: ", label)
        print(cm[i])
