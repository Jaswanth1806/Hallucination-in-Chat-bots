...
class TaskProcessor:
    ...
    def load(self, fold: str):
        ...
        for i, item in enumerate(dataset):
            ...
            # first label for Hallucination
            if "Hallucination" in label:
                label_int[0] = 1

            # second label for Entailment
            if "Entailment" in label:
                label_int[1] = 1

            # third label for Uncooperative
            if "Uncooperative" in label:
                label_int[2] = 1

            # fourth label for Generic
            if "Generic" in label:
                label_int[3] = 1
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), 'task21_model.pth')


    # For Testing
    model.load_state_dict(torch.load('task21_model.pth'))
